...
const ActionModal = ({ mode, onConfirm, onClose, loading, prefilledRate }) => {
  ...
  return (
    <>
      {/* ✅ Injecting a custom animation keyframe directly into the component 
        so it works instantly without touching your tailwind.config.js 
      */}
      <style>{`
        @keyframes scaleIn {
      ...
      <div
        ...
        onClick={onClose}
      >
        {/* Modal panel — ✅ Added animate-scaleIn here */}
        <div
          className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6 relative transform transition-all animate-scaleIn"
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    disabled // ✅ Completely disables the input
                    className="w-full px-4 py-2.5 border border-gray-200 rounded-full bg-gray-100 text-gray-500 cursor-not-allowed text-sm font-semibold transition-colors"
                  />
                  {/* We no longer need an onChange or a red border check since they can't touch it! */}
                </div>
              </div>
