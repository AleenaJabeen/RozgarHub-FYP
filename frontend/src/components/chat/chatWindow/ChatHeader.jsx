...
const ChatHeader = ({ otherUser, isTyping, isOnline, lastActiveAt, onBack }) => {
  return (
    <div className="sticky h-18 top-0 z-10 bg-white px-4 py-2 border-b border-gray-200 flex justify-between items-center shadow-sm">
      <div className="flex items-center gap-3">
        ...
        {otherUser?.avatar ? (
          <img src={otherUser?.avatar} alt="avatar" className="w-12 h-12 rounded-full object-cover bg-gray-200" />
        ) : (
        ...
        <div>
          <h3 className="font-bold text-gray-800">{capitalizeWords(otherUser?.name)}</h3>
          <p className={`text-xs font-medium truncate w-48 sm:w-auto ${
            isTyping || isOnline ? "text-secondary" : "text-gray-500"
          }`}>
            {isTyping ? "typing..." : isOnline ? "online" : formatLastSeen(lastActiveAt)}
          </p>
        </div>
      </div>
